...
type View = "Hoje" | "Plano mensal" | "Banco de conteúdo";
const nav: View[] = ["Hoje", "Plano mensal", "Banco de conteúdo"];
...
function Workspace({ session }: { session: Session }) {
  ...
  const loadPlan = useCallback(async () => {
    setLoading(true);
    try { const client = requireSupabase(); await syncAuroraPlanningBase(client); const [loadedPlan, loadedProduction] = await Promise.all([loadSeptember2026Plan(client), loadProduction(client)]); setPlan(loadedPlan); setProduction(loadedProduction); }
...
function LoadedPlan({ view, plan, production, importAurora, toggleStep }: { view: View; plan: PlannedPublication[]; production: ProductionContent[]; importAurora: () => void; toggleStep: (stepId: string, isDone: boolean) => void }) {
  ...
  if (view === "Hoje") return <Today rows={today} production={production} toggleStep={toggleStep} />;
  if (view === "Plano mensal") return <Month plan={plan} />;
  if (view === "Banco de conteúdo") return <Bank />;
  return null;
}

function Today({ rows, production, toggleStep }: { rows: PlannedPublication[]; production: ProductionContent[]; toggleStep: (stepId: string, isDone: boolean) => void }) { const actionable = production.flatMap((item) => item.steps).filter((step) => step.isRequired); const done = actionable.filter((step) => step.isDone).length; return <><p style={{ color: "#68707d", marginTop: -14, lineHeight: 1.55 }}>Sua mesa de trabalho: abra um canal para ver e concluir exatamente o que falta naquele conteúdo. Esta página não é uma lista de posts; é o foco necessário para produzir o plano.</p><div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(220px, 1fr))", gap: 12, margin: "25px 0 30px" }}><Metric value={`${done}/${actionable.length}`} label="etapas concluídas" note="em todos os conteúdos carregados"/><Metric value={String(actionable.length - done)} label="etapas restantes" note="para a produção em andamento"/></div><h2 style={{ fontSize: 19 }}>Foco de 04 de setembro</h2><div style={{ display: "grid", gap: 12 }}>{projectOrder.map((project) => <ProjectRows key={project} project={project} rows={rows.filter((row) => row.project === project)} production={production} toggleStep={toggleStep} />)}</div></> }

function ProjectRows({ project, rows, production, toggleStep }: { project: string; rows: PlannedPublication[]; production: ProductionContent[]; toggleStep: (stepId: string, isDone: boolean) => void }) { return <section style={{ background: "white", border: "1px solid #e8e6e1", borderRadius: 11, overflow: "hidden" }}><div style={{ padding: "12px 16px", borderLeft: `5px solid ${projectColor[project]}`, display: "flex", justifyContent: "space-between" }}><strong>{project}</strong><span style={{ color: "#68707d", fontSize: 12 }}>{rows.length} canal(is) no plano</span></div>{rows.map((row) => { const item = production.find((content) => content.project === project && content.title === row.title); const required = item?.steps.filter((step) => step.isRequired) ?? []; const done = required.filter((step) => step.isDone).length; return <details key={row.id} style={{ borderTop: "1px solid #e8e6e1" }}><summary style={{ cursor: item ? "pointer" : "default", padding: "13px 16px", listStyle: "none", display: "flex", justifyContent: "space-between", gap: 20, alignItems: "center" }}><div><strong style={{ fontSize: 14 }}>{row.channel}</strong><div style={{ fontSize: 12, color: "#68707d", marginTop: 3 }}>{row.title ?? "Ainda sem conteúdo vinculado"}</div></div>{item ? <span style={{ color: done === required.length ? "#227146" : "#8a5b08", fontSize: 12, fontWeight: 800 }}>{done}/{required.length} ETAPAS {done === required.length ? "✓" : ""}</span> : <span style={{ color: "#68707d", fontSize: 12 }}>AGUARDANDO FILA</span>}</summary>{item && <div style={{ borderTop: "1px solid #eee", padding: "14px 16px 18px" }}><p style={{ marginTop: 0, fontSize: 12, color: "#68707d" }}>{item.reference}</p>{["1 · Ideia", "2 · Construção", "3 · Publicação"].map((block) => <div key={block} style={{ marginTop: 14 }}><strong style={{ fontSize: 12 }}>{block}</strong><div style={{ display: "grid", gap: 7, marginTop: 7 }}>{item.steps.filter((step) => step.block === block).map((step) => <label key={step.id} style={{ display: "flex", alignItems: "center", gap: 9, cursor: "pointer", color: step.isDone ? "#227146" : "#303741", fontSize: 13 }}><input type="checkbox" checked={step.isDone} onChange={(event) => toggleStep(step.id, event.target.checked)} /><span style={{ textDecoration: step.isDone ? "line-through" : "none" }}>{step.label}{!step.isRequired ? " · opcional" : ""}</span></label>)}</div></div>)}</div>}</details> })}</section> }
