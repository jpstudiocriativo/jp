...
import { ensureSeptember2026Plan, loadSeptember2026Plan, type PlannedPublication } from "@/lib/supabase/plan";
...
function Workspace({ session }: { session: Session }) {
  ...
  return <main style={{ minHeight: "100vh", display: "grid", gridTemplateColumns: "245px 1fr", fontFamily: "Arial, sans-serif", background: "#f8f7f4", color: "#1f2630" }}>
    ...
    <section style={{ padding: "38px clamp(22px, 4vw, 64px)", maxWidth: 1450, width: "100%", margin: "0 auto" }}><header style={{ marginBottom: 28 }}><p style={{ margin: 0, color: "#68707d", fontSize: 14 }}>Setembro de 2026 · operação editorial</p><h1 style={{ margin: "8px 0 0", fontSize: 31, letterSpacing: -1 }}>{view}</h1></header>{message && <div style={{ padding: 12, borderRadius: 8, background: "#e7f5eb", color: "#227146", fontSize: 13, marginBottom: 18 }}>{message}</div>}{loading ? <p>Carregando plano…</p> : plan.length === 0 ? <EmptyPlan createPlan={createPlan} /> : <LoadedPlan view={view} plan={plan} />}</section>
  </main>;
}
...
function Production({ plan }: { plan: PlannedPublication[] }) { const linked = plan.length - plan.filter((row) => row.status === "Sem conteúdo").length; return <section style={{ maxWidth: 700 }}><h2 style={{ marginTop: 0 }}>Produção começa com o plano, não com tarefas soltas</h2><p style={{ color: "#68707d", lineHeight: 1.55 }}>Há {linked} conteúdo(s) vinculado(s) até agora. Quando você enviar as ideias de um projeto, elas se tornarão conteúdos-mãe e receberão o checklist correspondente.</p><div style={{ padding: 18, border: "1px dashed #d6d2ca", borderRadius: 10, background: "white" }}><strong>Próxima entrada esperada</strong><p style={{ marginBottom: 0, color: "#68707d", fontSize: 13 }}>Importar as 30 ideias da Aurora para YouTube. A importação vai vincular uma ideia a cada data e criar os blocos de produção de vídeo longo.</p></div></section> }
